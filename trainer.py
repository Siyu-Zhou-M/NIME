...
# 8.18 change
from Models.utils import plot_fine_tune_curves
...
class Self_Supervised_Trainer(BaseTrainer):
    ...
    def train_epoch(self, epoch_num=None):
        ...
        for i, batch in enumerate(self.pre_train_loader):
            X, targets, IDs = batch
            rep_mask, rep_mask_prediction, rep_contex, rep_target = self.model.pretrain_forward(X.to(self.device), IDs)
        ...
        if (epoch_num + 1) % 5 == 0:
            ...
            logger.info(f'Epoch {epoch_num} - Test_acc: {acc_test:.4f}')
...
def SS_train_runner(config, model, trainer, path):
    ...
    # from Models.model import starting_points_list, analyze_starting_points_distribution

    # if starting_points_list:
    #    print("Analyzing starting points distribution before training...")
    #    analyze_starting_points_distribution()
...
def Strain_runner(config, model, trainer, evaluator, path):
    ...
    for epoch in tqdm(range(start_epoch + 1, epochs + 1), desc='Training Epoch', leave=False):
        ...
        history['train_loss'].append(aggr_metrics_train['loss'])
        history['val_loss'].append(aggr_metrics_val['loss'])
        history['val_acc'].append(aggr_metrics_val['accuracy'])
        if 'AUROC' in aggr_metrics_val:
            history['val_auroc'].append(aggr_metrics_val['AUROC'])
    ...
    # 8.18 edit
    history_path = os.path.join(config['output_dir'], 'fine_tune_history.pkl')
    with open(history_path, 'wb') as f:
        pickle.dump(history, f)
    logger.info(f"Saved fine-tuning history to {history_path}")

    plot_fine_tune_curves(history, config['output_dir'])
