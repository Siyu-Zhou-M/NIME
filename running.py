...
def Rep_Learning(config, Data):
    ...
    SS_trainer = Self_Supervised_Trainer(Encoder, pre_train_loader, train_loader, test_loader, config, l2_reg=0,
    ...
    precision_lp = precision_score(test_labels.cpu().detach().numpy(), y_hat, average='weighted')
    recall_lp = recall_score(test_labels.cpu().detach().numpy(), y_hat, average='weighted')
    f1_lp = f1_score(test_labels.cpu().detach().numpy(), y_hat, average='weighted')
    auroc_lp = None
    if len(np.unique(test_labels.cpu().detach().numpy())) == 2:
        try:
            y_prob = clf.predict_proba(test_repr.cpu().detach().numpy())[:, 1]
            auroc_lp = roc_auc_score(test_labels.cpu().detach().numpy(), y_prob)
        except:
            pass

    print(f'Linear Probing Test_acc: {acc_test:.4f}')
    ...
    all_metrics['linear_probing_acc'] = acc_test
    all_metrics['linear_probing_precision'] = precision_lp
    all_metrics['linear_probing_recall'] = recall_lp
    all_metrics['linear_probing_f1'] = f1_lp
    all_metrics['linear_probing_auroc'] = auroc_lp
