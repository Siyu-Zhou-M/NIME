...
# 8.18 edit
def plot_fine_tune_curves(history, output_dir):
    ...
    if history['val_loss']:
        ...
        plt.axvline(x=best_val_loss_epoch, color='g', linestyle=':',
    ...
    if 'val_auroc' in history and history['val_auroc']:
        plt.plot(epochs, history['val_auroc'], 'm-', label='Validation AUROC', linewidth=2)

    if history['val_acc']:
        best_acc_idx = np.argmax(history['val_acc'])
        best_acc_epoch = epochs[best_acc_idx]
        plt.axvline(x=best_acc_epoch, color='b', linestyle=':',
    ...
    plot_path = os.path.join(output_dir, 'fine_tune_metrics.png')
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    plt.close()
    logger.info(f"Saved fine-tuning curves to {plot_path}")
